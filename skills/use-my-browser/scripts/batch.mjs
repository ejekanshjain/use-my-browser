...
let { page, context } = await connect();
...
for (let i = 0; i < steps.length; i++) {
  ...
  try {
    switch (step.op) {
      ...
      case "click":
        await requireLocator(page, step).click();
        break;
      case "fill":
        await requireLocator(page, step).fill(String(step.text ?? ""));
        break;
      case "type": {
        const text = String(step.text ?? "");
        const target = locator(page, step);
        if (target) {
          if (step.clear) await target.fill(text);
      ...
      case "check":
        await requireLocator(page, step).check();
        break;
      case "uncheck":
        await requireLocator(page, step).uncheck();
        break;
      case "hover":
        await requireLocator(page, step).hover();
        break;
      case "select": {
        const values = step.values ?? step.value;
        await requireLocator(page, step).selectOption(values);
        break;
      }
      case "wait": {
        const waits = [];
        if (step.url) waits.push(page.waitForURL(step.url));
        if (step.urlIncludes) {
          waits.push(
            page.waitForURL((href) => String(href).includes(step.urlIncludes)),
          );
        }
        if (step.text) {
          waits.push(
            page.getByText(step.text, { exact: step.exact }).waitFor(),
          );
        }
        if (step.load) waits.push(page.waitForLoadState(step.load));
        if (step.fn) waits.push(page.waitForFunction(step.fn));
        const target = locator(page, step);
        if (target) waits.push(target.waitFor());
        if (waits.length === 0) {
          throw new Error(
